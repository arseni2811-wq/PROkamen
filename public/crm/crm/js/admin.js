...
function renderMaterials() {
  ...
  const host = tbody.closest("section");
  if (host && !document.getElementById("adminMaterialSearch")) host.insertAdjacentHTML("afterbegin", '<div class="flex gap-2 mb-3"><input id="adminMaterialSearch" class="border rounded p-2 text-sm flex-1" placeholder="Бренд, артикул или название"><select id="adminMaterialFilter" class="border rounded p-2 text-sm"><option value="all">Все</option><option value="imported">Imported</option><option value="legacy">Legacy</option><option value="ready">Ready</option><option value="not-ready">Not ready</option><option value="out">OUT</option></select></div>');
  const query = String(document.getElementById("adminMaterialSearch")?.value || "").toLowerCase();
  const filter = document.getElementById("adminMaterialFilter")?.value || "all";
  const list = calculatorAdmin.materials.filter((material) => {
    const variants = material.variants || []; const imported = Boolean(material.importKey);
    const ready = variants.some((variant) => variant.availability?.ready);
    const text = [material.manufacturer, material.sku, material.title].join(" ").toLowerCase();
    return ["quartz", "granite", "onyx", "marble"].includes(material.category) && (!query || text.includes(query)) && (filter === "all" || (filter === "imported" && imported) || (filter === "legacy" && !imported) || (filter === "ready" && ready) || (filter === "not-ready" && imported && !ready) || (filter === "out" && variants.some((variant) => variant.discontinued)));
  });
  tbody.innerHTML = list.map((material) => { const variants = material.variants || []; const summary = variants.length ? `Variants: ${variants.length} · Ready: ${variants.filter(v => v.availability?.ready).length} · OUT: ${variants.filter(v => v.discontinued).length}` : "Legacy"; const details = variants.map((variant) => `<details class="mt-2 text-xs"><summary>${html([variant.commercialFormat, `${variant.lengthMm}×${variant.widthMm}×${variant.thicknessMm}`, variant.surface, variant.discontinued ? "OUT" : ""].filter(Boolean).join(" · "))} — ${html(variant.availability?.code || "—")}</summary><div class="p-2 bg-gray-50">${variant.sourcePrices.map((price) => `${html(price.fraction)}: ${(Number(price.amountMinor) / 100).toFixed(2)} ${html(price.currency)}${price.sourceLocation ? ` · ${html(price.sourceLocation)}` : ""}`).join("<br>")}<br><label><input class="variant-active" type="checkbox" ${variant.active ? "checked" : ""}> active</label> <label><input class="variant-public" type="checkbox" ${variant.publicAvailable ? "checked" : ""} ${variant.availability?.ready && !variant.discontinued ? "" : "disabled"}> public</label><button class="save-variant ml-2 underline" data-variant="${variant.materialVariantId}" data-material="${html(material.id)}">Сохранить</button></div></details>`).join(""); return `<tr class="border-b" data-material="${html(material.id)}"><td class="p-3"><strong>${html(material.title)}</strong><div class="text-xs text-gray-500">${html([material.manufacturer, material.sku, material.category, material.importKey ? "Imported" : "Legacy"].filter(Boolean).join(" · "))}</div><div class="text-xs mt-1">${html(summary)}</div>${details}</td><td class="p-3 font-mono text-xs">${html(material.id)}</td><td class="p-3">${(Number(material.basePriceUsdCents) / 100).toFixed(2)} USD</td><td class="p-3 text-right text-xs">${material.active ? "Активен" : "Скрыт"}${material.publicAvailable ? " · public" : ""}<br><button class="edit-material text-blue-700 underline mt-1">Изменить</button></td></tr>`; }).join("");
  document.getElementById("adminMaterialSearch")?.addEventListener("input", renderMaterials); document.getElementById("adminMaterialFilter")?.addEventListener("change", renderMaterials);
  tbody.querySelectorAll(".save-variant").forEach((button) => button.addEventListener("click", async () => { const detail = button.closest("details"); try { await api.updateCalculatorMaterialVariant(button.dataset.material, button.dataset.variant, { active: detail.querySelector(".variant-active").checked, publicAvailable: detail.querySelector(".variant-public").checked, publicSortOrder: 0 }); await loadAdmin(); } catch (error) { alert(`❌ ${error.message}`); } }));
